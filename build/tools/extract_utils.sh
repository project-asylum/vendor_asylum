...
function setup_vendor() {
    ...
    export ASYLUM_ROOT="$3"
    if [ ! -d "$ASYLUM_ROOT" ]; then
        echo "\$ASYLUM_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$ASYLUM_ROOT/$OUTDIR" ]; then
        mkdir -p "$ASYLUM_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$ASYLUM_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$ASYLUM_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$ASYLUM_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$ASYLUM_ROOT"/vendor/asylum/build/tools/smali/baksmali.jar
        export SMALIJAR="$ASYLUM_ROOT"/vendor/asylum/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$ASYLUM_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$ASYLUM_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$ASYLUM_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        DUMPDIR="$ASYLUM_ROOT"/system_dump
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$ASYLUM_ROOT"/vendor/asylum/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$ASYLUM_ROOT"/"$OUTDIR"/radio
